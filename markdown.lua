...
local function emphasis(text)
  ...
  do
    local function fl_rule(p1, p2, p3)
      if p1:match('^[%p%s]?$') and p3:match('^[%p%s]?$') then
        return p1 .. '<em>' .. p2 .. '</em>' .. p3
      else
        return p1 .. '_' .. p2 .. '_' .. p3
      end
    end
    text = text:gsub("(.?)_([^%s_])_(.?)", fl_rule)
    text = text:gsub("(.?)_(<strong>[^%s_]</strong>)_(.?)", fl_rule)
    text = text:gsub("(.?)_([^%s_][^<>_]-[^%s_])_(.?)", fl_rule)
    text = text:gsub("(.?)_([^<>_]-<strong>[^<>_]-</strong>[^<>_]-)_(.?)", fl_rule)
  end
  return text
end
